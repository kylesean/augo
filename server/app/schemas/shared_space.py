...
class CreateSpaceRequest(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=255, description="空间描述")


class UpdateSpaceRequest(BaseModel):
    """Request schema for updating a shared space."""

    name: Optional[str] = Field(None, min_length=1, max_length=255, description="空间名称")
    description: Optional[str] = Field(None, max_length=255, description="空间描述")
    status: Optional[str] = Field(None, pattern="^(active|archived)$", description="空间状态")
...
class SharedSpaceResponse(BaseModel):
    ...
    members: Optional[List[SpaceMemberResponse]] = Field(None, description="成员列表")
    transaction_count: int = Field(default=0, description="交易数量")
    current_invite_code: Optional[str] = Field(None, description="当前邀请码")
    invite_code_expires_at: Optional[datetime] = Field(None, description="邀请码过期时间")
